...
app.config.update(MAX_CONTENT_LENGTH=10*1024*1024,SESSION_COOKIE_HTTPONLY=True,SESSION_COOKIE_SAMESITE='Strict',SESSION_COOKIE_SECURE=os.getenv('COOKIE_SECURE','false')=='true')
...
def before():
    ...
    if request.method not in ['GET','HEAD','OPTIONS'] and request.headers.get('Origin'):
        origin=request.headers['Origin'];allowed={request.host_url.rstrip('/'),'http://localhost:4173','http://127.0.0.1:4173','http://localhost:5173','http://127.0.0.1:5173','http://terminal.local:4173'}
...
def admin():
    if not config.DEMO_MODE and (not config.ADMIN_TOKEN or not hmac.compare_digest(request.headers.get('X-Admin-Token',''),config.ADMIN_TOKEN)):raise Forbidden('Admin authentication is required.')
